...
def write_excel(out_path: str, trades_df: pd.DataFrame,
    ...
    if out.exists():
        try:
            ...
            print(f"\n⚠️  ERROR: Cannot write to file - it may be open in Excel")
            print(f"   Please close '{out.name}' and try again")
            print(f"\n   Alternative: Save to a different location")
            alt_path = out.parent / f"{out.stem}_new{out.suffix}"
            print(f"   Attempting to write to: {alt_path}")
            out = alt_path
    ...
    try:
        ...
        backup_path = os.path.join(desktop, out.name)
        ...
        print(f"   You can move this file to the correct location manually")
        ...
        if len(xccy_trades) > 0:
            ...
            if len(xccy_sample) > 0:
                ...
                for idx, row in xccy_sample.iterrows():
                    ...
                    print(f"    {c1}/{c2}: Leg1={fr1:.4f if pd.notna(fr1) else 'None'}, Leg2={fr2:.4f if pd.notna(fr2) else 'None'}")
